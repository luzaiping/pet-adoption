...
- `/pets` 列表页 —— 通过 `StatusBadge` 展示所有状态的宠物，切换物种筛选时会将页码重置为 1，每页展示 12 条记录；页面容器已统一为 `max-w-7xl` 并补齐响应式水平间距，320px 与 390px 下无横向溢出 —— 功能已验证，但实现质量弱于其他页面，后续值得进行一次代码审查走查。
- `/pets/[id]` 详情页 —— 采用 `await params` 模式，针对不存在的宠物调用 `notFound()` 并提供自定义的 `not-found.tsx` 页面，实现了图片画廊、物种/品种/年龄/性别/描述展示、收容所信息以及领养申请提交面板的对接 —— 除了次级缩略图代码路径尚未测试外（当前种子数据每只宠物仅分配了一张主图），其余均已通过验证。
...
- 根目录 `README.md` 已从 create-next-app 默认模板完整重写为面向客户和技术评审者的英文项目文档，包含线上演示入口、已交付功能、关键业务规则、架构与权限设计、技术栈、数据模型、本地运行、环境变量和部署说明。
- README 不包含内部学习过程、协作记录、后续规划或尚未实现的功能，也暂不嵌入截图；截图素材由开发者另行整理，演示视频及英文讲解仍待制作。
...
1. **录制并剪辑 Upwork Portfolio 演示视频，准备英文讲解与字幕。**
