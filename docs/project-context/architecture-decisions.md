...
### 19. Dashboard 根路径需要极简兜底页面,与"完整 Dashboard 首页设计"分离为两个独立任务
/dashboard 与 /dashboard/admin 在引入导航 Menu 组件的同时,补充极简的 page.tsx(欢迎语 + 功能入口链接,不含任何统计类数据查询),目的仅是避免 Menu 落地后根路径出现空白或 404 的体验断层。真正带信息密度的 Dashboard 首页内容(如待审核数量、我的申请状态摘要等)刻意不在本次任务范围内,留作独立任务,避免范围蔓延导致改动不可控。

### 20. Dashboard 菜单按角色分别定义为两组独立菜单项,不采用"USER 基础项 + ADMIN 追加项"的继承式设计
USER 角色显示"我的申请";ADMIN 角色显示"审核队列"。两者不存在包含关系——ADMIN 不显示"我的申请",因为按本系统的业务叙事,ADMIN 的角色定位是审核者而非领养申请人,让管理员账号出现"我的申请"入口在产品逻辑上不成立。这与架构决策 #19 中"极简兜底页面"的思路一致:USER 首页指向"我的申请",ADMIN 首页指向"审核队列",两套页面内容也各自独立,不共享同一套导航结构。
需要注意:这只是**展示层(Menu/首页入口)**的角色区分,不代表数据/校验层已经禁止 ADMIN 账号提交领养申请——submitApplicationAction 目前仍未对 role 做限制,这是讨论中识别出的一个潜在业务规则缺口,已知悉但本次不处理,留待后续单独评估是否需要在 Server Action 层面也加上角色限制。

### 21. /dashboard 根路径采用极简展示页方案,不采用角色跳转方案
/dashboard 与 /dashboard/admin 均落地为实际可见的展示页(欢迎语),而非登录后自动 redirect 到某个具体子页面。原因:(1) ADMIN 默认应优先跳转到"审核队列"还是未来的"宠物管理"这一判断,目前尚无足够依据(宠物管理功能未实现,无法比较使用频率),展示页避免过早锁定这个产品判断;(2) 当前每个角色下可用功能页数量还很少,跳转节省的点击成本有限,而展示页能让访问者(尤其是 Upwork 演示场景下的评审者)一眼看清角色划分与可用功能;(3) 展示页为架构决策 #19 中提到的"完整 Dashboard 首页"后续任务(待处理统计信息展示)预留了自然的内容承载位置,避免以后要把"跳转"改回"展示"。
