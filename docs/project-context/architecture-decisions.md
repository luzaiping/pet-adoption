...
计划：编写一个 `assertNotDemoMode()` 辅助函数，该函数由 `IS_DEMO` 环境变量控制，并在每个具有写路径的 Server Action 顶部调用，如果变量为 true 则抛出友好的错误提示。每日 Vercel Cron 定时任务（定在美东/欧洲夜间，即流量最低时段）会调用 `resetAndSeedDatabase()` 来清空并重新灌入数据。需要在 README 中记录的权衡是：如果管理员在夜间自动重置前不久刚生成了一个 AI 宠物简介，该简介将被回滚 —— 这是预期内的行为，并非 Bug。

状态：已部分实现。`IS_DEMO` 环境变量现已存在；首个实际应用是用于管控 `registerAction`（注册操作）。每日 Cron 重置以及用于其他写路径的 `assertNotDemoMode()` 目前仍在挂起（待开发）状态。
...
纵深防御计划包含 3 个层面（目前仅构建了第 1 层）：第 1 层 = 中间件的 `authorized()` 回调（已构建）；第 2 层 = `(dashboard)/dashboard/admin/layout.tsx` 中的角色检查（尚未构建）；第 3 层 = 在每个管理员 Server Action 内部调用的 `assertAdmin()` 辅助函数，其模式与计划中的 `assertNotDemoMode()` 相同（尚未构建 —— 延期至有实际的管理员 Server Action 需要挂载它们时再处理）。
