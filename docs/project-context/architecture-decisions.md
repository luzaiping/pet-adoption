...
20–30 张猫/狗的照片（4:3 比例，长边约 1200px，每张小于 300KB，源自 Pexels/Unsplash）直接提交到 `/public/pets/` 目录下，命名为 `dog-01.jpg`...`dog-15.jpg` / `cat-01.jpg`...`cat-15.jpg`，并提供一张其他物种使用的通用图片。管理员的“上传”UI 实际上是一个基于该固定图片池的单图选择器，而不是真实的上传管道。主表单只展示空状态或当前图片，Dog/Cat 候选图通过分页 Dialog 选择，避免图片池扩大后把所有缩略图铺在表单中；species 为空时图片保持为空，非空且不是 Dog/Cat 时才自动使用通用图片。该对应关系同时由 Zod schema 在服务端校验。`PetImage.url` 存储相对路径，每只宠物在当前范围内始终只保留一张 `isPrimary: true` 的图片。
...
### 23. 首页数据使用按请求的服务端快照，并通过组件级 Suspense 独立流式展示
首页统计和 Featured Pets 直接通过 Prisma 在 async Server Component 的数据路径中读取，不引入客户端轮询、WebSocket 或 TanStack Query；这里的“实时”定义为每次服务端请求读取当前数据库快照。公共布局现有的 `auth()` 已使首页成为动态服务端路由，因此不额外设置 `force-dynamic`，避免扩大缓存策略影响范围。

统计和 Featured Pets 各自拥有 Suspense 边界及布局匹配的骨架屏，使 Hero 不必等待 Neon 查询完成。统计的三项 count 在数据层内部并行执行。Featured Pets 固定选择最新 4 只 `AVAILABLE` 宠物，只读取一张主图，并使用 `createdAt desc + id desc` 确定性排序；它是稳定的展示规则而非随机或个性化推荐，因此 UI 使用 Featured 文案。数据库异常不会被捕获并伪装成数值 0，交由路由错误边界处理。

### 24：Pet 创建/编辑共用表单，species 归一化下沉至 Schema 层，create 模式禁止手动设置 status
PetForm 组件通过 mode: 'create' | 'edit' 复用同一套字段定义，避免维护两份几乎相同的表单。species 字段使用 shadcn Combobox，下拉选项来自 getDistinctSpecies() 的真实查询结果（而非硬编码），同时允许自由输入新值；输入值统一在 schemas/pets.ts 的 Zod transform 中做 trim + 首字母大写其余小写 归一化（仅覆盖单词场景，多单词 Title Case 留作已知限制），归一化逻辑下沉到 schema 层而非表单组件，以保证绕过前端直接调用 Server Action 时仍生效。shelterId 通过下拉选择，选项来自真实 shelter 表查询。Pet.status 字段只在 edit 模式的表单和 schema 中出现；create 路径下 Server Action 内部强制写入 AVAILABLE，不允许通过表单在创建阶段设置为 ADOPTED，以维持"宠物状态只能通过审批流程变更"的既有业务规则。updatePetSchema 采用全量字段更新（不使用 .partial()），与项目现有校验风格保持一致。

species 的归一化实现提取为 schema 与图片选择器共用的纯函数：schema transform 仍是归一化生效的权威入口，选择器则复用同一函数计算当前输入对应的图片池，避免在组件中复制规则。图片路径属于 create/update schema 的必填字段；schema 除了限制路径必须来自静态图片白名单，还会交叉校验图片池与归一化后的 species 一致。编辑时使用 Prisma nested write 原子删除旧图片并创建新的主图，保证单图约束。

### 25：管理员宠物列表使用独立查询模型、URL 筛选和分层 Loading Boundary

管理员列表的数据读取使用独立的 `getPetsForAdmin()`，不继续扩展公共 `getPets()`。原因是管理端需要 name 模糊搜索、age 精确筛选、species/status 筛选、createdAt 排序以及不同的字段投影；将这些条件加入公共查询会使面向用户的简单浏览职责与后台管理职责耦合。两个查询可以继续复用相同的分页规模和确定性排序原则，但分别维护各自的筛选能力。

管理列表由 URL `searchParams` 驱动，支持 `name` / `age` / `species` / `status` / `sort` / `page`。应用任意搜索、筛选或排序时必须移除 page，使结果回到第一页；分页链接则完整保留当前查询条件。桌面端使用 Table，移动端使用 Card，两种布局消费同一份服务端查询结果和统一格式化后的 row view model。当前页面只提供新建、编辑和查询入口，不增加删除或批量操作。

筛选 UI 按设备重新组织，而不是把桌面控件简单纵向堆叠：桌面端将 name 主搜索与操作按钮放在第一行，Age/Species/Status/Created date 等宽放在第二行；移动端默认只展示 name 搜索和 Filters 入口，高级条件放入 Dialog，应用后统一更新 URL。

`/dashboard/admin/pets/loading.tsx` 只负责列表骨架；`new/loading.tsx` 与 `[id]/edit/loading.tsx` 通过更近的路由层级覆盖父级 fallback，并共同复用 `PetFormSkeleton`。Loading UI 必须匹配目标页面结构，不能让创建/编辑页显示表格骨架。

---

### 26：页面渲染阶段的基础设施异常由根级 `app/error.tsx` 统一兜底

所有从页面或嵌套路由布局中冒泡出来的未捕获渲染异常，统一交给 `src/app/error.tsx` 展示通用、可重试且不泄露内部信息的错误页面。数据库连接失败不得在各查询函数中转换为空数据，否则会把基础设施故障伪装成合法空状态。错误页面只向用户提供通用说明、`reset()` 重试和返回首页入口；原始错误仅用于日志。

当前不创建 `global-error.tsx`：它用于处理根 `app/layout.tsx` 自身的异常，而根布局不执行数据库查询。Server Action 和客户端事件中的预期失败不属于该渲染边界，继续由各 Action 的返回值或 mutation `onError` 处理。
